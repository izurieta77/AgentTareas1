...
class GroundedGenerator:
    ...
    def generate_response(self, query: str, contexts: List[ParentChunk]) -> Dict[str, Any]:
        ...
        system_prompt = (
            ...
            "1. NO INVENTES NI SUPONGAS NADA: cada afirmación de tu respuesta debe estar respaldada por el contexto.\n"
            "2. El contexto puede estar en un idioma distinto al de la pregunta (p. ej. inglés). Traducir o parafrasear "
            "información que SÍ está en el contexto es válido y obligatorio: no es motivo para rechazar la respuesta.\n"
            "3. Solo si el contexto no contiene NINGUNA información relevante para la pregunta, responde ÚNICAMENTE con la "
            "frase: \"INFORMACIÓN NO DISPONIBLE EN EL CORPUS DE SEGURIDAD\". No añadas nada más.\n"
            "4. Si la pregunta tiene varias partes y el contexto solo cubre algunas, responde las partes disponibles e "
            "indica explícitamente que el resto no consta en el corpus.\n"
            "5. El contexto puede mezclar documentos de distintos dispositivos: responde usando únicamente los contextos "
            "del dispositivo por el que se pregunta (fíjate en el nombre de documento de cada cabecera de contexto).\n"
            "6. DEBES citar la fuente de cada afirmación usando el formato estricto: [NombreDocumento] o [ID: chunk_id] basándote en las cabeceras de contexto.\n"
            "7. Prohibido usar conocimientos externos a los contextos proveídos.\n"
            "8. Sé directo, técnico y conciso."
        )
